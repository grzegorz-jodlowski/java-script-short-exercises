...
function fromStringToSec(input) {
  let seconds = 0;
  input.split(':').reverse().forEach((el, i) => {
    i === 0 ? seconds += Number(el) : seconds += Number(el) * Math.pow(60, i);
  })
  return seconds;
}

function fromSecToObject(seconds) {
  return {
...
function totalDuration() {
  ...
  lessons.map(lesson => {
    if (lesson.name.includes('[1x')) {
      duration.Adam += fromStringToSec(lesson.length);
    } else if (lesson.name.includes('[2x')) {
      duration.Przemek += fromStringToSec(lesson.length);
    } else if (lesson.name.includes('[3x')) {
      duration.Marcin += fromStringToSec(lesson.length);
    } else {
      duration.Other += fromStringToSec(lesson.length);
    }
  })
  ...
  for (let name in duration) {
    if (duration.hasOwnProperty(name)) {
      totalDuration += duration[name];
    }
  }


  return `Kurs Opanuj JavaScript trwa ${fromSecToObject(totalDuration).hours} godzin i ${fromSecToObject(totalDuration).minutes} minut.
  Moduł Adama: ${fromSecToObject(duration.Adam).hours} godzin ${fromSecToObject(duration.Adam).minutes} minut
  Moduł Przemka: ${fromSecToObject(duration.Przemek).hours} godzin ${fromSecToObject(duration.Przemek).minutes} minut
  Moduł Marcina: ${fromSecToObject(duration.Marcin).hours} godzin ${fromSecToObject(duration.Marcin).minutes} minut`
}
